...
PDF_PATH = "exam_tests/practice_final.pdf"
...
siglip_model = SiglipModel.from_pretrained(SIGLIP_ID).to(DEVICE)
...
class Block:
    ...
    text: str
    type: str = "Text"  # to distinguish figures from text
...
def is_meaningful_figure(pil_image) -> bool:
    labels = [
        "a diagram, graph, flowchart, or organizational grid table", # Positive
        "a block of text",                                           # Negative
        "a single line of text or header",                           # Negative
        "blank white space"                                          # Negative
    ]
    ...
    try:
        ...
        score_diagram = probs[0][0].item()
        score_text_block = probs[0][1].item()
        score_header = probs[0][2].item()
        score_blank = probs[0][3].item()
        
        # we specifically want to toss page headers and empty crops
        if score_header > 0.6 or score_blank > 0.6:
            return False
        
        # if its not a header or blank, and it has at least some diagram signal
        # we keep it to be safe
        return score_diagram > 0.1 
...
def parse_pdf_to_questions(pdf_path: Path) -> List[Question]:
    ...
    end = min(END_PAGE or last_page, last_page)
    model = lp.Detectron2LayoutModel(
        'lp://PrimaLayout/mask_rcnn_R_50_FPN_3x/config',
        extra_config=["MODEL.ROI_HEADS.SCORE_THRESH_TEST", 0.5],
        label_map={1: "Text", 2: "Image", 3: "Table", 4: "Math", 5: "Separator", 6: "Other"}
    )
    ...
    for page_idx in range(start - 1, end):
        ...
        for b in layout:
            if b.type not in ["Text", "Image", "Table", "Math"]:
                continue

            x1, y1, x2, y2 = map(int, b.block.coordinates)
            
            rejected_dir = Path(OUTPUT_DIR) / "rejected_by_siglip"
            rejected_dir.mkdir(parents=True, exist_ok=True)

            if b.type in ["Image", "Table", "Math"]:
                pad = 15
                ...
                if is_meaningful_figure(crop):
                    ...
                    # save what was rejected
                    reject_filename = f"p{page_num}_{y1}_rejected_{b.type}.png"
                    crop.save(rejected_dir / reject_filename)
                    print(f"  [TOSS] SigLIP rejected {b.type} at {y1} - Saved to {rejected_dir}")
                continue

            # text logic
            text = ocr_crop(bgr, (x1, y1, x2, y2))
            if not text or len(text) < 3: # ignore tiny noise
                continue
        ...
        for block in page_blocks:
            if block.type == "Text" and is_question_start(block.text):
